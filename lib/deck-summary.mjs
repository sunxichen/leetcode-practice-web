...
export const DECK_SUMMARY_VERSION = 2;

/**
 * @param {{
 *   hot100: { id: string }[],
 *   interview: import('./interview-types').InterviewCard[],
 *   resume?: import('./interview-types').InterviewCard[],
 * }} decks
 * @returns {object} 可直接序列化的摘要
 */
export function buildDeckSummary({ hot100, interview, resume }) {
  return {
    ...
    decks: {
      ...
      // resume 题集还没有卡片时不产出键——首页读方对缺失的题集不编造计数。
      ...(resume
        ? {
            resume: {
              cardCount: resume.length,
              cardIds: resume.map((c) => c.id),
              byCategory: countBy(resume, CATEGORIES, (c) => c.category),
              byPriority: countBy(resume, PRIORITIES, (c) => c.priority),
            },
          }
        : {}),
    },
